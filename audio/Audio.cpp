...
void Source::updateEffects()
{
	...
	{
		auxiliaryEffectSlot->applyToSource(source);
	}
	else alSource3i(source,AL_AUXILIARY_SEND_FILTER, AL_EFFECTSLOT_NULL, 0, AL_FILTER_NULL);
	if(filter)
	{
		alSourcei(source, AL_DIRECT_FILTER, filter->getFilter());
	}
	else alSourcei(source, AL_DIRECT_FILTER, AL_FILTER_NULL);
}
